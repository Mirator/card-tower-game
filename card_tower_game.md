...
- Resource rows use simple shared icon glyphs (brick / sword / crystal) instead of text abbreviations
- Center battlefield with simple castle silhouettes that scale by castle value, single wall lines that scale by wall value (hidden at zero), hidden tower labels for cleaner focus, played-card travel, enemy reveal, and impact feedback
